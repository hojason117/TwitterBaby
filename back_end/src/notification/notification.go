...
func (h *Handler) GetConnection(c echo.Context) (err error) {
	...
	id, err := uuid.NewV1()
	cc := client{id, username, conn, make(chan model.Notification)}
...
func (manager *clientManager) FlushNotif(conn *client) {
	...
	sort.Slice(target.Notifications, func(i, j int) bool {
		return target.Notifications[i].Timestamp.Before(target.Notifications[j].Timestamp)
	})
